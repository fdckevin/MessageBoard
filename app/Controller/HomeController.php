	...
	public function changePassword() {
		...
		if($this->request->isAjax()) {
			...
			if($this->request->is('post')) {
				...
				$loggedUserID = $this->Auth->user()[0]['Users']['id'];

				$password = AuthComponent::password($this->request->data['pass1']);

				$user = $this->Users->find('count', array(
					'conditions' => array('Users.id' => $loggedUserID, 'Users.password' => $password)
				));

				if($user==1) {

					$this->Users->read(null, $loggedUserID);

					$this->Users->set(array(
						'password' => AuthComponent::password($this->request->data['pass2'])
					));

					if($this->Users->save()) {
						return json_encode(array('success' => 1, 'message' => 'Password successfully changed'));
					}  
